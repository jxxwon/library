	...
	<div class = "noticeFormContainer inner pageContent_mt">
		...
		<div class = "noticeFormContent">
			...
			<div class="noticeContainer">
				<form action="freeUpdateProc?no=${free.no }" method="post" id="f" class="noticeWrite">
					<table class="noticeWriteForm">
						<tr>
							<th>제목</th>
							<td colspan=4><input type = "text" id = "title" name = "title" value = "${free.title }"></td>
						</tr>
						<tr>
							<th>작성자</th>
							<td>${free.writer }</td>
							<th class="right" style = "border-left:1px solid #ddd;">작성일</th>
							<td width="200">${free.writeDate}</td>
						</tr>
						<tr>
							<th>내용</th>
							<td colspan=4><textarea id = "content" name = "content" rows ="20" cols ="15" style = "resize: none">${free.content }</textarea></td>
						</tr>
					</table>
					<div class="noticeBtn">
						<input type = "button" value = "수정" onclick="writeChk()" >
						<input type = "button" value = "취소" onclick="location.href='freeContent?no=${free.no}'">
					</div>
				</form>
			</div>
		</div>
